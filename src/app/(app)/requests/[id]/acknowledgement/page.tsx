  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
        ...
        >
          <span aria-hidden>←</span> {t("Back to this request")}
        </Link>
        <button
          type="button"
      ...
      <article className="mt-5 rounded-2xl border border-line bg-white p-8 shadow-[var(--shadow-panel)] print:rounded-none print:border-0 print:p-0 print:shadow-none">
        <header className="border-b-2 border-navy-900 pb-5">
          ...
          <p className="mt-2 text-sm text-ink-2">
            {t("Issued under section 7(1). This is a record of an application received; it is not a reply to it.")}
          </p>
        </header>

        <Row label={t("Registration number")} value={c.registrationNumber} mono />
        <Row label={t("Date of receipt")} value={formatDate(v.submittedOn)} />
        <Row label={t("Public authority")} value={`${c.authority.office}, ${c.authority.ministry}`} />
        <Row
          label={t("Date forwarded to the CPIO")}
        ...
        {c.applicant ? (
          <>
            <Row label={t("Name")} value={c.applicant.name} />
            <Row label={t("Address")} value={c.applicant.address} />
            <Row label={t("Email")} value={c.applicant.email} />
            {c.applicant.mobile ? (
              <Row label={t("Mobile")} value={c.applicant.mobile} />
            ) : null}
            <Row
              label={t("Declaration")}
              value={
          ...
          <Row label={t("Name")} value="As given in the application" />
        )}

        <Section title={t("Information sought")} />
        <p className="mt-3 whitespace-pre-line text-sm leading-7 text-ink">
          {c.question}
        </p>
        {c.format ? (
          <Row
            label={t("Form in which access was sought")}
            value={`${ACCESS_FORMAT_COPY[c.format]} (section 7(9))`}
          />
        ) : null}

        <Section title={t("Fee")} />
        <Row label={t("Application fee")} value={feeLabelOf(c.fee)} />
        {c.fee.receiptNumber ? (
          <Row label={t("Receipt number")} value={c.fee.receiptNumber} mono />
        ) : null}
        {c.fee.paidOn ? (
          <Row label={t("Received on")} value={formatDate(new Date(`${c.fee.paidOn}T00:00:00`))} />
        ) : null}
        {c.fee.waived && c.fee.waiverBasis ? (
          <Row label={t("Basis of waiver")} value={`${c.fee.waiverBasis} (proviso to section 7(5))`} />
        ) : null}
        {c.additionalFee ? (
          <Row
        ...
        <Section title={t("Time allowed for a reply")} />
        <Row label={t("Period allowed")} value={v.d.clock.spec.label} />
        <Row label={t("Basis")} value={v.d.clock.spec.basis} />
        <Row label={t("Reply due on or before")} value={formatDate(v.expectedBy)} />
        <p className="mt-3 text-sm leading-7 text-ink-2">
          {t("If no reply is given within that period, the failure is treated as a refusal under section 7(2), and an appeal may be filed free of cost.")}
        </p>

        <Section title={t("Officers")} />
        ...
        <Section title={t("Right of appeal")} />
        <p className="mt-3 text-sm leading-7 text-ink-2">
