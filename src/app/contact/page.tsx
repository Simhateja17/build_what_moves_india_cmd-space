...
const SUPPORT_OPTIONS: Array<{
  ...
  {
    title: "Call Us",
    primary: "1800 123 4789",
    href: "tel:18001234789",
    secondary: ["Mon – Fri (9:00 AM – 6:00 PM)", "(Toll Free)"],
    icon: "phone",
  },
  {
    title: "Email Us",
    primary: "support@rtionline.gov.in",
    href: "mailto:support@rtionline.gov.in",
    secondary: ["We will respond within", "1–2 working days"],
    icon: "mail",
  },
  {
    title: "Live Chat",
    primary: "Chat with our support executive",
    secondary: ["Mon – Fri (9:00 AM – 6:00 PM)"],
    icon: "chat",
  },
  {
    title: "Write to Us",
    primary: "RTI Online Support Desk",
    secondary: ["Department of Personnel & Training,", "North Block, New Delhi – 110001"],
    icon: "location",
  },
];
      ...
      <main id="main" className="flex-1">
        <div className="mx-auto w-full max-w-[1320px] px-4 py-7 sm:px-8 sm:py-9 lg:px-10">
          ...
          <header className="mt-5 grid items-center gap-6 border-b border-line-2 pb-7 sm:grid-cols-[1fr_310px]">
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-navy-900 sm:text-4xl">{t("Contact Us")}</h1>
              <p className="mt-3 text-[15px] text-ink-2">{t("Assistance is available. Contact any of the channels below.")}</p>
          ...
          <section className="mt-7" aria-labelledby="support-options-title">
            <h2 id="support-options-title" className="text-lg font-bold text-navy-900">{t("Support options")}</h2>
            <div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              {SUPPORT_OPTIONS.map((option) => (
                <article key={option.title} className="rounded-2xl border border-line bg-white px-5 py-6 text-center shadow-[var(--shadow-panel)]">
                  ...
                  <h3 className="mt-4 text-lg font-bold text-navy-900">{t(option.title)}</h3>
                  <div className="mt-2 text-sm leading-5 text-ink-2">
                    {option.href ? <a href={option.href} className="font-bold text-navy-700 hover:underline">{option.primary}</a> : t(option.primary)}
                  </div>
                  <div className="mt-3 text-xs leading-5 text-ink-2">
                    {option.secondary.map((line) => <span key={line} className="block">{t(line)}</span>)}
                  </div>
                </article>
              ))}
            </div>
          ...
          <section className="mt-8" aria-labelledby="message-title">
            <h2 id="message-title" className="text-lg font-bold text-navy-900">{t("Send a message")}</h2>
            <form onSubmit={submit} className="mt-4 rounded-2xl border border-line bg-white p-5 shadow-[var(--shadow-panel)] sm:p-7">
              <div className="grid gap-5 md:grid-cols-2 md:gap-8">
                <div className="space-y-5">
                  <label className="field-label">{t("Full Name")}
                    <input name="name" required defaultValue={citizenName} placeholder={t("Enter your full name")} className="field-input" />
                  </label>
                  <label className="field-label">{t("Email Address")}
                    <input name="email" type="email" required placeholder={t("Enter your email address")} className="field-input" />
                  </label>
                  <label className="field-label">{t("Subject")}
                    <select name="subject" required defaultValue="" className="field-input">
                      <option value="" disabled>{t("Select a subject")}</option>
                      <option>{t("Application status")}</option>
                      <option>{t("Payment issue")}</option>
                      <option>{t("First Appeal")}</option>
                      <option>{t("Technical problem")}</option>
                      <option>{t("Other")}</option>
                    </select>
                  </label>
                </div>
                <div className="flex flex-col">
                  <label className="field-label">{t("Application Number")} <span className="font-normal text-muted">{t("(Optional)")}</span>
                    <input name="applicationNumber" placeholder={t("Enter application number")} className="field-input" />
                  </label>
                  <label className="mt-5 flex flex-1 flex-col field-label">{t("Message")}
                    <textarea name="message" required rows={6} placeholder={t("Type your message here...")} className="field-input min-h-36 flex-1 resize-y" />
                  </label>
              ...
              {submitted ? (
                <div role="status" className="mt-5 rounded-xl border border-govgreen-600/25 bg-govgreen-50 px-4 py-3 text-sm font-semibold text-govgreen-700">
                  {t("The message has been sent. A response will be provided within 1–2 working days.")}
                </div>
              ) : null}
            </form>
          </section>
          ...
          <aside className="mt-7 flex flex-col gap-4 rounded-2xl border border-navy-600/15 bg-navy-50 p-5 sm:flex-row sm:items-center">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-bold text-navy-900">{t("Before submitting a query, the FAQ section may be checked.")}</p>
              <p className="mt-1 text-sm text-ink-2">{t("You may find answers to common questions there.")}</p>
            </div>
            <Link href="/faq" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-white px-5 py-2.5 text-sm font-bold text-navy-700 shadow-sm hover:bg-navy-50">{t("View FAQ")} <span aria-hidden>→</span></Link>
          </aside>
        </div>
      </main>
      <GovFooter />
