...
export function ActionRequired({ items }: { items: ActionItem[] }) {
  ...
  return (
    <section
      ...
      className="rounded-[28px] border border-line bg-surface p-4 shadow-[var(--shadow-panel)] sm:p-6"
    >
      ...
      <ul className="grid gap-3 sm:grid-cols-2">
        {items.map((a) => (
          <li
            key={a.id}
            className={`flex min-h-[205px] flex-col rounded-2xl border border-line-2 border-l-4 bg-canvas/45 p-4 transition hover:-translate-y-0.5 hover:bg-white hover:shadow-[var(--shadow-panel)] ${BORDER[a.tone]}`}
          >
            ...
            <div className="mt-auto pt-4">
              <Link
                href={a.href}
                className={`block w-full rounded-lg px-4 py-2.5 text-center text-[13px] font-bold transition sm:w-fit ${BUTTON[a.tone]}`}
              >
                {a.cta}
              </Link>
            </div>
